...
//console.log(template);
	...
	var changePriceSku = (function () {
		...
		var obj = {
			price     : '.js-skuPrice',
			salePrice : '.js-actSkuPrice',
			quantity  : '.js-availQuantity',
			youSave   : '.js-you_save'
		};
		...
		function setSkuPrice( e ) {
			...
			$( obj.youSave ).html( '<span class="color-orange">' + save + '</span> (' + discount + '&percnt;)' );
	...
	$.fn.quantity = function ( options ) {
		...
		var setTotal = function () {
			th.on( 'keyup mouseup click', options.item, function () {
				clearTimeout( _timeout );
				var _this = this;
				_timeout  = setTimeout( function () {
					var price             = parseFloat( $( _this ).data( 'price' ) ),
						currency          = $( _this ).data( 'currency' ),
						price_shipping    = $( _this ).data( 'price_shipping' ),
						currency_shipping = $( _this ).data( 'currency_shipping' ),
						quantity          = parseInt( $( _this ).val() );

					adsCart.convPriceTotal( {
						price             : price,
						currency          : currency,
						price_shipping    : price_shipping,
						currency_shipping : currency_shipping,
						quantity          : quantity

					}, function ( price ) {
						th.find( options.total ).text( price );
					} );
				}, 400 );
			} );
		};
...
jQuery( function ( $ ) {
	...
	zoomer = (function () {
		...
		var $productSlider = $( productSlider ).data( 'fotorama' );
		var zoomLarge      = '.product-content-left .wrap-meta';
		var zoomLargeBox   = ('<div class="zoomer"><div class="zoomer-large"></div></div>');
		var zoomScaleBox   = ('<div class="zoomer-scale"></div>');
		...
		return {
			init        : function () {
				...
				$( productSlider ).on( 'fotorama:show', function () {
					$this.setImg( $this.getImg() );
				} );
				$( productSlider ).on( 'fotorama:ready', function () {
					$( zoomLarge ).append( zoomLargeBox );
					$( productSlider + ' .fotorama__stage' ).append( zoomScaleBox );
					$this.setImg( $this.getImg() );
				} );

				function endZoom( e ) {
					$( '.zoomer-large' ).hide();
					$( '.zoomer-scale' ).css( 'opacity', '' );
				}
				...
				$( productSlider + ' .fotorama__stage' ).on( {
			...
			setImg      : function ( url ) {
				$( '.zoomer-large' ).css( "background", "url(" + url + ") no-repeat" );
			},
			getImg      : function () {
				return $productSlider.activeFrame.img;
			},
			allowedShow : function () {
...
(function ( $ ) {
	...
	var pageSingleProduct = (function () {
		...
		function setTimeShipping( option ) {
			$( '.js-shipping-info' ).html( option.data( 'info' ) );
		}

		function setTotalPrice( option ) {
			$( '.js-single-quantity' )
				.data( 'price_shipping', option.data( 'price' ) )
				.click();
		}
		...
		return {
			init : function () {
				$this = this;
				$( window ).load( function () {
					$( '.js-single-quantity' ).click();
				} );

				$( 'select.js-single-shipping' ).on( 'change', function () {
